...
class CreditHelperUpdateController extends BaseController {
  ...
  final Rxn<CreditHelperModel> getOneCreditHelper = Rxn<CreditHelperModel>();

  var creditHelperId = 0.obs;
  ...
  Future<void> fetchGetOneCreditHelper(int id) async {
    try {
      var creditHelper = await creditHelperRepository.getOneCreditHelper(id);
      if (creditHelper != null) {
        getOneCreditHelper.value = creditHelper;
        setDetails(creditHelper);
      }
    } catch (e) {
      showError('خطا در دریافت اطلاعات', 'خطا در دریافت اطلاعات اعتبار: ${e.toString()}');
    }
  }

  // تنظیم جزئیات اعتبار در فرم
  void setDetails(CreditHelperModel model) {
    ...
    if (model.startDate != null) {
      startDateController.text = model.startDate?.toPersianDate(showTime: true,digitType: NumStrLanguage.English) ?? "";
    }

    if (model.endDate != null) {
      endDateController.text = model.endDate?.toPersianDate(showTime: true,digitType: NumStrLanguage.English) ?? "";
    }
  }

  // ویرایش اعتبار
  Future<void> updateCreditHelper() async {
    try {
      ...
      if (startDateController.text.isNotEmpty) {
        gregorianStartDate = convertJalaliToGregorianCustomDate(startDateController.text);
      }

      if (endDateController.text.isNotEmpty) {
        gregorianEndDate = convertJalaliToGregorianCustomDate(endDateController.text);
      }
      ...
      var response = await creditHelperRepository.updateCreditHelper(
        id: creditHelperId.value,
      ...
      if (response != null) {
        ...
        if (response['infos'] != null && response['infos'].isNotEmpty) {
          ...
          Get.snackbar('موفقیت', 'اعتبار با موفقیت ویرایش شد',
              titleText: Text('موفقیت',
                textAlign: TextAlign.center,
                style: TextStyle(color: AppColor.textColor),),
              messageText: Text(
                  'اعتبار با موفقیت ویرایش شد', textAlign: TextAlign.center,
                  style: TextStyle(color: AppColor.textColor)));
      ...
      showError('خطا در ویرایش اعتبار', 'خطا: ${e.toString()}');
      print('خطا در ویرایش اعتبار: $e');
    } finally {
      EasyLoading.dismiss();
      isLoading.value = false;
    }
