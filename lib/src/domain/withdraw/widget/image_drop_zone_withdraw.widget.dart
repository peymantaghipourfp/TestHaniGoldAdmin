...
class ImageDropZoneWithdraw extends StatefulWidget {
  final WithdrawCreateController controller;
  final bool isDesktop;

  const ImageDropZoneWithdraw({
    super.key,
  ...
  @override
  State<ImageDropZoneWithdraw> createState() => _ImageDropZoneWithdrawState();
}

class _ImageDropZoneWithdrawState extends State<ImageDropZoneWithdraw> {
  ...
  void _setupWebDragAndDrop() {
    // Add global drag and drop listeners for web
  ...
  void _handleDragEnter(html.Event event) {
    event.preventDefault();
    event.stopPropagation();
    // Only set drag over if we're over our drop zone
  ...
  void _handleDragLeave(html.Event event) {
    event.preventDefault();
    event.stopPropagation();
    // Only clear drag over if we're leaving our drop zone
    if (_isOverDropZone(event)) {
      _setDragOver(false);
    }
  }
  ...
  void _handleDrop(html.Event event) {
    event.preventDefault();
    event.stopPropagation();
    // Only handle drop if it's over our drop zone
    if (_isOverDropZone(event)) {
  ...
  void _handleFileDrop(html.Event event) async {
    try {
      ...
      // Cast to dynamic to access dataTransfer property
      ...
      if (files != null && files.length > 0) {
        ...
        if (xFiles.isNotEmpty) {
          widget.controller.handleDroppedFiles(xFiles);
        } else {
  ...
  Widget build(BuildContext context) {
    return Container(padding: EdgeInsets.only(right: 30),
  ...
  Widget _buildWebDropZone() {
    return Stack(
      children: [
        // Invisible overlay for web drag and drop
        Positioned.fill(
          child: MouseRegion(
          ...
          children: [
            ...
            Text(
              _isDragOver
                  ...
                  : 'فایل‌های تصویری را اینجا بکشید یا کلیک کنید',
            ...
            Text(
              'فرمت‌های پشتیبانی شده: JPG, PNG, GIF, BMP, WebP',
              style: AppTextStyle.labelText.copyWith(
