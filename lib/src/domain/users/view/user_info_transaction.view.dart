...
class _UserInfoTransactionViewState extends State<UserInfoTransactionView> {
  ...
  Future<void> _onIssueInvoicePressed(
    ...
    final inventoryId = trans.parentId ?? trans.id;
    if (inventoryId == null || inventoryId == 0) {
      Get.snackbar(
        'خطا',
        'شناسه فاکتور موجود نیست',
        titleText: Text(
          'خطا',
    ...
    if (inventory == null) return; // fetch already reported the error
    ...
    if (details.isEmpty) {
      Get.snackbar(
        'اطلاعات',
        'ردیفی برای صدور فاکتور یافت نشد',
        titleText: Text(
          'اطلاعات',
          textAlign: TextAlign.center,
          style: TextStyle(color: AppColor.textColor),
